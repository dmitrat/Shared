...
using OutWit.Shared.Email.Plugins;                  // IEmailProviderPlugin
...
| Package | Key | Notes |
|---|---|---|
| [`OutWit.Shared.Email.Plugin.Null`](../OutWit.Shared.Email.Plugin.Null/) | `Null` | Zero-config fallback. Two modes: `LogOnly` (prints message to logs — useful in dev) and `Drop` (fails fast — useful in production-without-email). |
